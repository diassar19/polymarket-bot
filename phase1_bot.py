...
class ProbabilityEstimator:
    ...
    def estimate(self, market: Market) -> Optional[tuple[float, float, str]]:
        ...
        IMPORTANT: This default implementation is a PLACEHOLDER.
        It uses simple heuristics that probably won't be profitable.
        Replace with your own data-driven models.
        """

        # Strategy 0: Weather model (data-driven, first pass)
        try:
            weather_result = self.weather_model.try_estimate(market)
            if weather_result is not None:
                return weather_result
        except Exception:
            pass  # Fall through to existing strategies

        # Strategy 1: Arbitrage check — YES + NO should = 1.00
        # If they don't, there's a structural mispricing
        price_sum = market.yes_price + market.no_price
        if price_sum < 0.97:
            # Prices sum to less than $1 — both sides are "cheap"
            # This is rare on Polymarket's CLOB but worth checking
            gap = 1.0 - price_sum
            return (
                market.yes_price + gap/2,  # Split the gap
                0.9,
                f"Structural arbitrage: YES+NO={price_sum:.3f}, gap={gap:.3f}"
            )
        
        if price_sum > 1.03:
            # Prices sum to more than $1 — both sides are "expensive"
            gap = price_sum - 1.0
            return (
    ...
    def run_once(self):
        ...
        for signal in signals:
            ...
            self.db.log_forecast(
                ...
                source="weather" if "WeatherModel" in signal.reasoning else "heuristic",
            )
...
ENVIRONMENT VARIABLES (for live trading in Phase 3):
